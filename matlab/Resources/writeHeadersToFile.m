...
for i= 2:length(signalNameArray)
    tabbedNextSignalName = [dlm, char(signalNameArray(1, i))];                         % Add tab delimiter before signal name
    signalNamesString = strcat(signalNamesString, tabbedNextSignalName);                   % Concatenate signal names delimited by a tab.
    tabbedNextSignalFormat = [dlm, char(signalFormatArray(1, i))];                     % Add tab delimiter before signal format
    signalFormatsString = strcat(signalFormatsString, tabbedNextSignalFormat);             % Concatenate signal formats delimited by a tab.
    tabbedNextSignalUnit = [dlm, char(signalUnitArray(1, i))];                         % Add tab delimiter before signal unit
